 ...
 *
 * <p>Address format: {@code host:port} (e.g. {@code currency-converter:9000}).
 * Plaintext only — TLS would be added at the edge / mesh layer.
 *
 * <p>The deadline is applied per-call inside the adapter (see
 * {@link GrpcCurrencyConverterClient}), not here, so we keep the stub bean
 * reusable.
 */
@Configuration
public class GrpcConfig {
    ...
    private static HostPort parseHostPort(String raw) {
        // Accept either "host:port" or "static://host:port" so the same env
        // var works whether or not we later add a discovery scheme.
